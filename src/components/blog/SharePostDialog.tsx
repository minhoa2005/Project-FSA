      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-2xl z-50 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white z-10 flex items-center justify-between p-4 border-b">
          ...
          <button
            onClick={onClose}
            disabled={submitting}
            className="w-9 h-9 rounded-full hover:bg-gray-100 flex items-center justify-center text-gray-600 disabled:opacity-50"
          >
            <X className="w-6 h-6" />
          </button>
        </div>
          ...
          <div className="mx-4 mb-4 border rounded-lg overflow-hidden bg-gray-50">
            ...
            <div className="p-3 space-y-2">
              ...
              {originalPost.media && originalPost.media.length > 0 && (
                <div className="grid grid-cols-2 gap-1 mt-2">
                  {originalPost.media.slice(0, 4).map((media) => (
                    <div key={media.id} className="relative aspect-video bg-gray-200 rounded overflow-hidden">
                      {media.type === "image" ? (
                        <Image
                          src={media.url}
                          alt=""
                        ...
                        <div className="w-full h-full flex items-center justify-center bg-gray-300">
                          ...
                          <span className="absolute bottom-1 right-1 text-xs bg-black/60 text-white px-1 rounded">Video</span>
                        </div>
                      ) : null}
                    </div>
                  ))}
              ...
              {originalPost.media && originalPost.media.length > 4 && (
                <p className="text-xs text-gray-500">+{originalPost.media.length - 4} ảnh/video khác</p>
              )}
            </div>
          </div>
